...
# 3. Sidebar (Restored Mock Mode here)
with st.sidebar:
    st.header("Settings")
    ...
    st.divider()
    # RESTORED: The dev_mode checkbox
    dev_mode = st.checkbox("🚀 Mock Mode (No API calls)", value=False, help="Use this for testing the UI without using your quota.")
...
if (st.session_state.api_key or dev_mode) and (uploaded_file or dev_mode):
    ...
    pages = get_pdf_text(uploaded_file) if uploaded_file else ["Mock Data Page"]
    ...
    if not st.session_state.current_question:
        if st.button("Sensei, ask me a question!", type="primary"):
            if dev_mode:
                # Mock Response
                st.session_state.current_question = "Japanese: 休みは何曜日ですか？\nRomaji: Yasumi wa nan-youbi desu ka?"
                st.rerun()
            else:
    ...
    # --- SCREEN 2: Practice & Feedback ---
        ...
        if student_audio and not st.session_state.feedback:
            if st.button("Submit Answer"):
                if dev_mode:
                    # Mock Feedback
                    st.session_state.feedback = "Japanese: よくできました！正解です。\nRomaji: Yoku dekimashita! Seikai desu."
                    st.rerun()
                else:
    ...
    st.warning("Please enter an API Key and upload a PDF to begin, or enable Mock Mode in the sidebar.")
